...
import { getConversationHistory, saveMessage, clearOldHistory } from '../db/index.js';
...
function isLowSignalItem(item) {
  ...
  if (/^(your government|police jury|public works|contact us|follow us)$/i.test(title)) return true;
...
async function getFreshCivicContext() {
  const now = Date.now();
  if (cachedContext.items.length > 0 && now - cachedContext.at < CONTEXT_TTL_MS) {
...
function buildChatContext(items) {
  if (!items || items.length === 0) return 'No current civic context available.';
...
Skip promotional content or opinions`;
  ...
  try {
    ...
    return digest || 'VotePlease: Weekly digest unavailable. Reply HELP for support.';
  } catch (err) {
